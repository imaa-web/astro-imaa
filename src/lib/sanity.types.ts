/**
 * ---------------------------------------------------------------------------------
 * This file has been generated by Sanity TypeGen.
 * Command: `sanity typegen generate`
 *
 * Any modifications made directly to this file will be overwritten the next time
 * the TypeScript definitions are generated. Please make changes to the Sanity
 * schema definitions and/or GROQ queries if you need to update these types.
 *
 * For more information on how to use Sanity TypeGen, visit the official documentation:
 * https://www.sanity.io/docs/sanity-typegen
 * ---------------------------------------------------------------------------------
 */

export declare const internalGroqTypeReferenceTo: unique symbol;

// Source: schema.json
export type Address = {
  street?: string;
  complement?: string;
  neighborhood?: string;
  cityState?: string;
  zipCode?: string;
};

export type SanityImageAssetReference = {
  _ref: string;
  _type: "reference";
  _weak?: boolean;
  [internalGroqTypeReferenceTo]?: "sanity.imageAsset";
};

export type BlockContent = Array<
  | {
      children?: Array<{
        marks?: Array<string>;
        text?: string;
        _type: "span";
        _key: string;
      }>;
      style?: "normal" | "h2" | "h3" | "h4" | "h5" | "h6" | "blockquote";
      listItem?: "bullet" | "number";
      markDefs?: Array<{
        href: string;
        blank?: boolean;
        _type: "link";
        _key: string;
      }>;
      level?: number;
      _type: "block";
      _key: string;
    }
  | {
      asset?: SanityImageAssetReference;
      media?: unknown;
      hotspot?: SanityImageHotspot;
      crop?: SanityImageCrop;
      alt: string;
      caption?: string;
      _type: "image";
      _key: string;
    }
  | ({
      _key: string;
    } & HorizontalRule)
  | ({
      _key: string;
    } & DownloadableFile)
  | ({
      _key: string;
    } & VideoFile)
  | ({
      _key: string;
    } & YoutubeEmbed)
  | ({
      _key: string;
    } & ProjectShowcaseSection)
>;

export type ProjectShowcaseSection = {
  _type: "projectShowcaseSection";
  heading: string;
  subtitle?: string;
  items: Array<
    {
      _key: string;
    } & ProjectShowcaseItem
  >;
};

export type ProjectShowcaseItem = {
  _type: "projectShowcaseItem";
  name: string;
  description: string;
  cta?: Cta;
};

export type CtaSection = {
  _type: "ctaSection";
  heading: string;
  subheading?: string;
  primaryCta?: Cta;
  secondaryCta?: Cta;
  background: "primary" | "muted";
};

export type FormField = {
  _type: "formField";
  fieldType: "input" | "textarea" | "select" | "checkbox" | "radio";
  label: string;
  placeholder?: string;
  inputType?: "text" | "email" | "tel" | "number";
  required?: boolean;
  width?: "half" | "full";
  options?: Array<{
    label: string;
    _key: string;
  }>;
};

export type PageReference = {
  _ref: string;
  _type: "reference";
  _weak?: boolean;
  [internalGroqTypeReferenceTo]?: "page";
};

export type HomePageReference = {
  _ref: string;
  _type: "reference";
  _weak?: boolean;
  [internalGroqTypeReferenceTo]?: "homePage";
};

export type ContactPageReference = {
  _ref: string;
  _type: "reference";
  _weak?: boolean;
  [internalGroqTypeReferenceTo]?: "contactPage";
};

export type Cta = {
  _type: "cta";
  label?: string;
  linkType?: "internal" | "external";
  pageReference?: PageReference | HomePageReference | ContactPageReference;
  externalUrl?: string;
  openInNewTab?: boolean;
};

export type SocialLink = {
  _type: "socialLink";
  platform: "instagram" | "facebook" | "youtube" | "twitter" | "bluesky";
  url: string;
  label?: string;
  image?: {
    asset?: SanityImageAssetReference;
    media?: unknown;
    hotspot?: SanityImageHotspot;
    crop?: SanityImageCrop;
    alt?: string;
    _type: "image";
  };
};

export type ContactInfo = {
  _type: "contactInfo";
  address?: Address;
  phones?: Array<{
    label?: string;
    number: string;
    isWhatsApp?: boolean;
    _key: string;
  }>;
  emails?: Array<{
    label?: string;
    address: string;
    _key: string;
  }>;
};

export type SanityFileAssetReference = {
  _ref: string;
  _type: "reference";
  _weak?: boolean;
  [internalGroqTypeReferenceTo]?: "sanity.fileAsset";
};

export type VideoFile = {
  _type: "videoFile";
  file: {
    asset?: SanityFileAssetReference;
    media?: unknown;
    _type: "file";
  };
  posterImage?: {
    asset?: SanityImageAssetReference;
    media?: unknown;
    hotspot?: SanityImageHotspot;
    crop?: SanityImageCrop;
    _type: "image";
  };
  caption?: string;
  autoplay?: boolean;
};

export type YoutubeEmbed = {
  _type: "youtubeEmbed";
  url: string;
  caption?: string;
};

export type DownloadableFile = {
  _type: "downloadableFile";
  asset?: SanityFileAssetReference;
  media?: unknown;
  title?: string;
  label?: string;
};

export type HomeMissionSection = {
  _type: "homeMissionSection";
  label: string;
  heading: string;
  headingHighlight?: string;
  description: string;
  secondaryDescription?: string;
  image?: {
    asset?: SanityImageAssetReference;
    media?: unknown;
    hotspot?: SanityImageHotspot;
    crop?: SanityImageCrop;
    alt?: string;
    _type: "image";
  };
  pillars?: Array<{
    icon: "music" | "heart" | "users" | "star";
    title: string;
    description: string;
    color?:
      | "primary"
      | "secondary"
      | "accent"
      | "primaryLight"
      | "secondaryDark"
      | "muted";
    _type: "pillar";
    _key: string;
  }>;
};

export type HomeProjectsPreview = {
  _type: "homeProjectsPreview";
  label: string;
  heading: string;
  image?: {
    asset?: SanityImageAssetReference;
    media?: unknown;
    hotspot?: SanityImageHotspot;
    crop?: SanityImageCrop;
    alt?: string;
    _type: "image";
  };
  projects?: Array<{
    title: string;
    description: string;
    accent?: "secondary" | "primary" | "primaryLight" | "accent" | "muted";
    _type: "project";
    _key: string;
  }>;
};

export type HomeBlockHighlightSection = {
  _type: "homeBlockHighlightSection";
  label: string;
  milestone: string;
  milestoneLabel: string;
  heading: string;
  body: string;
  footer?: string;
};

export type HorizontalRule = {
  _type: "horizontalRule";
  info?: string;
};

export type FeaturedImage = {
  _type: "featuredImage";
  asset?: SanityImageAssetReference;
  media?: unknown;
  hotspot?: SanityImageHotspot;
  crop?: SanityImageCrop;
  alt?: string;
  showInPage?: boolean;
};

export type Gallery = {
  _type: "gallery";
  heading?: string;
  layout: "grid2" | "grid3" | "masonry" | "carousel";
  autoplay?: boolean;
  autoplayInterval?: number;
  images: Array<{
    asset?: SanityImageAssetReference;
    media?: unknown;
    hotspot?: SanityImageHotspot;
    crop?: SanityImageCrop;
    alt?: string;
    caption?: string;
    _type: "image";
    _key: string;
  }>;
};

export type TextWithIllustration = {
  _type: "textWithIllustration";
  heading?: string;
  text: string;
  cta?: Cta;
  image?: {
    asset?: SanityImageAssetReference;
    media?: unknown;
    hotspot?: SanityImageHotspot;
    crop?: SanityImageCrop;
    alt?: string;
    _type: "image";
  };
  imagePosition: "left" | "right";
};

export type RichText = {
  _type: "richText";
  content: BlockContent;
  background: "default" | "muted";
};

export type Hero = {
  _type: "hero";
  heading?: string;
  tagline?: string;
};

export type MenuItem = {
  _type: "menuItem";
  pageReference?: PageReference | HomePageReference | ContactPageReference;
  label?: string;
  submenu?: Array<
    {
      _key: string;
    } & MenuItem
  >;
};

export type Menu = {
  _type: "menu";
  label: string;
  items?: Array<
    {
      _key: string;
    } & MenuItem
  >;
};

export type TransparencySection = {
  _id: string;
  _type: "transparencySection";
  _createdAt: string;
  _updatedAt: string;
  _rev: string;
  title: string;
  slug: Slug;
  partner: string;
  order?: number;
  hero?: Hero;
  description?: BlockContent;
  projects?: Array<{
    title: string;
    origin?: string;
    value?: string;
    status?: "in_progress" | "completed" | "in_analysis" | "pending";
    documents?: Array<
      {
        _key: string;
      } & DownloadableFile
    >;
    _type: "project";
    _key: string;
  }>;
  documents?: Array<
    {
      _key: string;
    } & DownloadableFile
  >;
  excerpt: string;
};

export type Slug = {
  _type: "slug";
  current: string;
  source?: string;
};

export type SiteSettings = {
  _id: string;
  _type: "siteSettings";
  _createdAt: string;
  _updatedAt: string;
  _rev: string;
  title: string;
  defaultSeoDescription?: string;
  logo?: {
    asset?: SanityImageAssetReference;
    media?: unknown;
    hotspot?: SanityImageHotspot;
    crop?: SanityImageCrop;
    alt?: string;
    _type: "image";
  };
  mainMenu?: Menu;
  footerMenu?: Menu;
  socialLinks?: Array<
    {
      _key: string;
    } & SocialLink
  >;
  contactInfo?: ContactInfo;
};

export type SanityImageCrop = {
  _type: "sanity.imageCrop";
  top: number;
  bottom: number;
  left: number;
  right: number;
};

export type SanityImageHotspot = {
  _type: "sanity.imageHotspot";
  x: number;
  y: number;
  height: number;
  width: number;
};

export type Page = {
  _id: string;
  _type: "page";
  _createdAt: string;
  _updatedAt: string;
  _rev: string;
  title: string;
  slug: Slug;
  hero?: Hero;
  featuredImage?: FeaturedImage;
  excerpt: string;
  pageBuilder?: Array<
    | ({
        _key: string;
      } & RichText)
    | ({
        _key: string;
      } & TextWithIllustration)
    | ({
        _key: string;
      } & CtaSection)
    | ({
        _key: string;
      } & Gallery)
    | ({
        _key: string;
      } & VideoFile)
    | ({
        _key: string;
      } & YoutubeEmbed)
    | ({
        _key: string;
      } & HorizontalRule)
  >;
};

export type ContactPage = {
  _id: string;
  _type: "contactPage";
  _createdAt: string;
  _updatedAt: string;
  _rev: string;
  heading: string;
  tagline?: string;
  seoDescription: string;
  contactTabLabel: string;
  contactFormHeading?: string;
  contactFormDescription?: string;
  enrollmentActive?: boolean;
  enrollmentTabLabel: string;
  enrollmentFormHeading?: string;
  enrollmentFormDescription?: string;
  enrollmentFormFields: Array<
    {
      _key: string;
    } & FormField
  >;
};

export type HomePage = {
  _id: string;
  _type: "homePage";
  _createdAt: string;
  _updatedAt: string;
  _rev: string;
  title: string;
  excerpt: string;
  logo?: {
    asset?: SanityImageAssetReference;
    media?: unknown;
    hotspot?: SanityImageHotspot;
    crop?: SanityImageCrop;
    alt?: string;
    _type: "image";
  };
  heroDescription: string;
  primaryCta?: Cta;
  secondaryCta?: Cta;
  blockHighlightSection?: HomeBlockHighlightSection;
  missionSection?: HomeMissionSection;
  projectsPreview?: HomeProjectsPreview;
};

export type MediaTag = {
  _id: string;
  _type: "media.tag";
  _createdAt: string;
  _updatedAt: string;
  _rev: string;
  name?: Slug;
};

export type SanityImagePaletteSwatch = {
  _type: "sanity.imagePaletteSwatch";
  background?: string;
  foreground?: string;
  population?: number;
  title?: string;
};

export type SanityImagePalette = {
  _type: "sanity.imagePalette";
  darkMuted?: SanityImagePaletteSwatch;
  lightVibrant?: SanityImagePaletteSwatch;
  darkVibrant?: SanityImagePaletteSwatch;
  vibrant?: SanityImagePaletteSwatch;
  dominant?: SanityImagePaletteSwatch;
  lightMuted?: SanityImagePaletteSwatch;
  muted?: SanityImagePaletteSwatch;
};

export type SanityImageDimensions = {
  _type: "sanity.imageDimensions";
  height: number;
  width: number;
  aspectRatio: number;
};

export type SanityImageMetadata = {
  _type: "sanity.imageMetadata";
  location?: Geopoint;
  dimensions?: SanityImageDimensions;
  palette?: SanityImagePalette;
  lqip?: string;
  blurHash?: string;
  thumbHash?: string;
  hasAlpha?: boolean;
  isOpaque?: boolean;
};

export type SanityFileAsset = {
  _id: string;
  _type: "sanity.fileAsset";
  _createdAt: string;
  _updatedAt: string;
  _rev: string;
  originalFilename?: string;
  label?: string;
  title?: string;
  description?: string;
  altText?: string;
  sha1hash: string;
  extension: string;
  mimeType: string;
  size: number;
  assetId: string;
  uploadId?: string;
  path: string;
  url: string;
  source?: SanityAssetSourceData;
};

export type SanityAssetSourceData = {
  _type: "sanity.assetSourceData";
  name?: string;
  id?: string;
  url?: string;
};

export type SanityImageAsset = {
  _id: string;
  _type: "sanity.imageAsset";
  _createdAt: string;
  _updatedAt: string;
  _rev: string;
  originalFilename?: string;
  label?: string;
  title?: string;
  description?: string;
  altText?: string;
  sha1hash: string;
  extension: string;
  mimeType: string;
  size: number;
  assetId: string;
  uploadId?: string;
  path: string;
  url: string;
  metadata?: SanityImageMetadata;
  source?: SanityAssetSourceData;
};

export type Geopoint = {
  _type: "geopoint";
  lat?: number;
  lng?: number;
  alt?: number;
};

export type AllSanitySchemaTypes =
  | Address
  | SanityImageAssetReference
  | BlockContent
  | ProjectShowcaseSection
  | ProjectShowcaseItem
  | CtaSection
  | FormField
  | PageReference
  | HomePageReference
  | ContactPageReference
  | Cta
  | SocialLink
  | ContactInfo
  | SanityFileAssetReference
  | VideoFile
  | YoutubeEmbed
  | DownloadableFile
  | HomeMissionSection
  | HomeProjectsPreview
  | HomeBlockHighlightSection
  | HorizontalRule
  | FeaturedImage
  | Gallery
  | TextWithIllustration
  | RichText
  | Hero
  | MenuItem
  | Menu
  | TransparencySection
  | Slug
  | SiteSettings
  | SanityImageCrop
  | SanityImageHotspot
  | Page
  | ContactPage
  | HomePage
  | MediaTag
  | SanityImagePaletteSwatch
  | SanityImagePalette
  | SanityImageDimensions
  | SanityImageMetadata
  | SanityFileAsset
  | SanityAssetSourceData
  | SanityImageAsset
  | Geopoint;

// Source: ../astro-imaa/src/lib/queries/contact.ts
// Variable: CONTACT_PAGE_QUERY
// Query: *[_type == "contactPage"][0] {    heading,    tagline,    seoDescription,    contactTabLabel,    contactFormHeading,    contactFormDescription,    enrollmentActive,    enrollmentTabLabel,    enrollmentFormHeading,    enrollmentFormDescription,    "enrollmentFormFields": enrollmentFormFields[] {      _key,      fieldType,      label,      placeholder,      inputType,      required,      width,      "options": options[] {        _key,        label      }    }  }
export type CONTACT_PAGE_QUERY_RESULT = {
  heading: string;
  tagline: string | null;
  seoDescription: string;
  contactTabLabel: string;
  contactFormHeading: string | null;
  contactFormDescription: string | null;
  enrollmentActive: boolean | null;
  enrollmentTabLabel: string;
  enrollmentFormHeading: string | null;
  enrollmentFormDescription: string | null;
  enrollmentFormFields: Array<{
    _key: string;
    fieldType: "checkbox" | "input" | "radio" | "select" | "textarea";
    label: string;
    placeholder: string | null;
    inputType: "email" | "number" | "tel" | "text" | null;
    required: boolean | null;
    width: "full" | "half" | null;
    options: Array<{
      _key: string;
      label: string;
    }> | null;
  }>;
} | null;

// Source: ../astro-imaa/src/lib/queries/home.ts
// Variable: HOME_PAGE_QUERY
// Query: *[_id == "homePage"][0] {    title,    excerpt,    heroDescription,    "logo": logo {        _key,  hotspot,  crop,  alt,  caption,  asset-> {    _id,    url,    altText,    metadata {      lqip,      dimensions {        width,        height,        aspectRatio      }    }  }    },    "primaryCta": primaryCta {   label,  linkType,  "slug": select(  pageReference->_type == "homePage" => "inicio",  pageReference->_type == "contactPage" => "contato",  pageReference->slug.current),  externalUrl,  openInNewTab },    "secondaryCta": secondaryCta {   label,  linkType,  "slug": select(  pageReference->_type == "homePage" => "inicio",  pageReference->_type == "contactPage" => "contato",  pageReference->slug.current),  externalUrl,  openInNewTab },    "projectsPreview": projectsPreview {      label,      heading,      "image": image {   _key,  hotspot,  crop,  alt,  caption,  asset-> {    _id,    url,    altText,    metadata {      lqip,      dimensions {        width,        height,        aspectRatio      }    }  } },      projects[] {        _key,        title,        description,        accent      },    },    "missionSection": missionSection {      label,      heading,      headingHighlight,      description,      secondaryDescription,      "image": image {   _key,  hotspot,  crop,  alt,  caption,  asset-> {    _id,    url,    altText,    metadata {      lqip,      dimensions {        width,        height,        aspectRatio      }    }  } },      pillars[] {        _key,        icon,        title,        description,        color      },    },    "blockHighlightSection": blockHighlightSection {      label,      milestone,      milestoneLabel,      heading,      body,      footer,    },  }
export type HOME_PAGE_QUERY_RESULT =
  | {
      title: null;
      excerpt: null;
      heroDescription: null;
      logo: null;
      primaryCta: null;
      secondaryCta: null;
      projectsPreview: null;
      missionSection: null;
      blockHighlightSection: null;
    }
  | {
      title: string | null;
      excerpt: null;
      heroDescription: null;
      logo: null;
      primaryCta: null;
      secondaryCta: null;
      projectsPreview: null;
      missionSection: null;
      blockHighlightSection: null;
    }
  | {
      title: string;
      excerpt: null;
      heroDescription: null;
      logo: {
        _key: null;
        hotspot: SanityImageHotspot | null;
        crop: SanityImageCrop | null;
        alt: string | null;
        caption: null;
        asset: {
          _id: string;
          url: string;
          altText: string | null;
          metadata: {
            lqip: string | null;
            dimensions: {
              width: number;
              height: number;
              aspectRatio: number;
            } | null;
          } | null;
        } | null;
      } | null;
      primaryCta: null;
      secondaryCta: null;
      projectsPreview: null;
      missionSection: null;
      blockHighlightSection: null;
    }
  | {
      title: string;
      excerpt: string;
      heroDescription: null;
      logo: null;
      primaryCta: null;
      secondaryCta: null;
      projectsPreview: null;
      missionSection: null;
      blockHighlightSection: null;
    }
  | {
      title: string;
      excerpt: string;
      heroDescription: string;
      logo: {
        _key: null;
        hotspot: SanityImageHotspot | null;
        crop: SanityImageCrop | null;
        alt: string | null;
        caption: null;
        asset: {
          _id: string;
          url: string;
          altText: string | null;
          metadata: {
            lqip: string | null;
            dimensions: {
              width: number;
              height: number;
              aspectRatio: number;
            } | null;
          } | null;
        } | null;
      } | null;
      primaryCta: {
        label: string | null;
        linkType: "external" | "internal" | null;
        slug: string | "contato" | "inicio" | null;
        externalUrl: string | null;
        openInNewTab: boolean | null;
      } | null;
      secondaryCta: {
        label: string | null;
        linkType: "external" | "internal" | null;
        slug: string | "contato" | "inicio" | null;
        externalUrl: string | null;
        openInNewTab: boolean | null;
      } | null;
      projectsPreview: {
        label: string;
        heading: string;
        image: {
          _key: null;
          hotspot: SanityImageHotspot | null;
          crop: SanityImageCrop | null;
          alt: string | null;
          caption: null;
          asset: {
            _id: string;
            url: string;
            altText: string | null;
            metadata: {
              lqip: string | null;
              dimensions: {
                width: number;
                height: number;
                aspectRatio: number;
              } | null;
            } | null;
          } | null;
        } | null;
        projects: Array<{
          _key: string;
          title: string;
          description: string;
          accent:
            | "accent"
            | "muted"
            | "primary"
            | "primaryLight"
            | "secondary"
            | null;
        }> | null;
      } | null;
      missionSection: {
        label: string;
        heading: string;
        headingHighlight: string | null;
        description: string;
        secondaryDescription: string | null;
        image: {
          _key: null;
          hotspot: SanityImageHotspot | null;
          crop: SanityImageCrop | null;
          alt: string | null;
          caption: null;
          asset: {
            _id: string;
            url: string;
            altText: string | null;
            metadata: {
              lqip: string | null;
              dimensions: {
                width: number;
                height: number;
                aspectRatio: number;
              } | null;
            } | null;
          } | null;
        } | null;
        pillars: Array<{
          _key: string;
          icon: "heart" | "music" | "star" | "users";
          title: string;
          description: string;
          color:
            | "accent"
            | "muted"
            | "primary"
            | "primaryLight"
            | "secondary"
            | "secondaryDark"
            | null;
        }> | null;
      } | null;
      blockHighlightSection: {
        label: string;
        milestone: string;
        milestoneLabel: string;
        heading: string;
        body: string;
        footer: string | null;
      } | null;
    }
  | null;

// Source: ../astro-imaa/src/lib/queries/page.ts
// Variable: ALL_PAGES_QUERY
// Query: *[_type == "page" && defined(slug.current)] | order(_createdAt desc) {        _id,  title,  excerpt,    "slug": slug.current,    hero {    heading,    tagline,  },  "featuredImage": featuredImage {   _key,  hotspot,  crop,  alt,  caption,  asset-> {    _id,    url,    altText,    metadata {      lqip,      dimensions {        width,        height,        aspectRatio      }    }  }, showInPage },  "pageBuilder": pageBuilder[] {   _key,  _type,  _type == "ctaSection" => {    ...,    "primaryCta": primaryCta {   label,  linkType,  "slug": select(  pageReference->_type == "homePage" => "inicio",  pageReference->_type == "contactPage" => "contato",  pageReference->slug.current),  externalUrl,  openInNewTab },    "secondaryCta": secondaryCta {   label,  linkType,  "slug": select(  pageReference->_type == "homePage" => "inicio",  pageReference->_type == "contactPage" => "contato",  pageReference->slug.current),  externalUrl,  openInNewTab }  },  _type == "textWithIllustration" => {    ...,    "image": image {   _key,  hotspot,  crop,  alt,  caption,  asset-> {    _id,    url,    altText,    metadata {      lqip,      dimensions {        width,        height,        aspectRatio      }    }  } },    "cta": cta {   label,  linkType,  "slug": select(  pageReference->_type == "homePage" => "inicio",  pageReference->_type == "contactPage" => "contato",  pageReference->slug.current),  externalUrl,  openInNewTab }  },  _type == "gallery" => {    ...,    "images": images[] {   _key,  hotspot,  crop,  alt,  caption,  asset-> {    _id,    url,    altText,    metadata {      lqip,      dimensions {        width,        height,        aspectRatio      }    }  } },    autoplay,    "autoplayInterval": coalesce(autoplayInterval, 4)  },  _type == "videoFile" => {   ...,  "file": {    "url": file.asset->url,    "mimeType": file.asset->mimeType  },  "posterImage": posterImage {   _key,  hotspot,  crop,  alt,  caption,  asset-> {    _id,    url,    altText,    metadata {      lqip,      dimensions {        width,        height,        aspectRatio      }    }  } } },  _type == "youtubeEmbed" => {   ...,  url,  caption },  _type == "horizontalRule" => {    ...  },  _type == "richText" => {    ...,    content[] {   ...,  _type == "image" => {   _key,  hotspot,  crop,  alt,  caption,  asset-> {    _id,    url,    altText,    metadata {      lqip,      dimensions {        width,        height,        aspectRatio      }    }  } },  _type == "videoFile" => {   ...,  "file": {    "url": file.asset->url,    "mimeType": file.asset->mimeType  },  "posterImage": posterImage {   _key,  hotspot,  crop,  alt,  caption,  asset-> {    _id,    url,    altText,    metadata {      lqip,      dimensions {        width,        height,        aspectRatio      }    }  } } },  _type == "youtubeEmbed" => {   ...,  url,  caption },  _type == "downloadableFile" => {   ...,  "url": asset->url },  _type == "projectShowcaseSection" => {   ...,  items[] {    _key,    name,    description,    "cta": cta {   label,  linkType,  "slug": select(  pageReference->_type == "homePage" => "inicio",  pageReference->_type == "contactPage" => "contato",  pageReference->slug.current),  externalUrl,  openInNewTab }  } }, },    background,  }, }  }
export type ALL_PAGES_QUERY_RESULT = Array<{
  _id: string;
  title: string;
  excerpt: string;
  slug: string;
  hero: {
    heading: string | null;
    tagline: string | null;
  } | null;
  featuredImage: {
    _key: null;
    hotspot: SanityImageHotspot | null;
    crop: SanityImageCrop | null;
    alt: string | null;
    caption: null;
    asset: {
      _id: string;
      url: string;
      altText: string | null;
      metadata: {
        lqip: string | null;
        dimensions: {
          width: number;
          height: number;
          aspectRatio: number;
        } | null;
      } | null;
    } | null;
    showInPage: boolean | null;
  } | null;
  pageBuilder: Array<
    | {
        _key: string;
        _type: "ctaSection";
        heading: string;
        subheading?: string;
        primaryCta: {
          label: string | null;
          linkType: "external" | "internal" | null;
          slug: string | "contato" | "inicio" | null;
          externalUrl: string | null;
          openInNewTab: boolean | null;
        } | null;
        secondaryCta: {
          label: string | null;
          linkType: "external" | "internal" | null;
          slug: string | "contato" | "inicio" | null;
          externalUrl: string | null;
          openInNewTab: boolean | null;
        } | null;
        background: "muted" | "primary";
      }
    | {
        _key: string;
        _type: "gallery";
        heading?: string;
        layout: "carousel" | "grid2" | "grid3" | "masonry";
        autoplay: boolean | null;
        autoplayInterval: number | 4;
        images: Array<{
          _key: string;
          hotspot: SanityImageHotspot | null;
          crop: SanityImageCrop | null;
          alt: string | null;
          caption: string | null;
          asset: {
            _id: string;
            url: string;
            altText: string | null;
            metadata: {
              lqip: string | null;
              dimensions: {
                width: number;
                height: number;
                aspectRatio: number;
              } | null;
            } | null;
          } | null;
        }>;
      }
    | {
        _key: string;
        _type: "horizontalRule";
        info?: string;
      }
    | {
        _key: string;
        _type: "richText";
        content: Array<
          | {
              children?: Array<{
                marks?: Array<string>;
                text?: string;
                _type: "span";
                _key: string;
              }>;
              style?:
                | "blockquote"
                | "h2"
                | "h3"
                | "h4"
                | "h5"
                | "h6"
                | "normal";
              listItem?: "bullet" | "number";
              markDefs?: Array<{
                href: string;
                blank?: boolean;
                _type: "link";
                _key: string;
              }>;
              level?: number;
              _type: "block";
              _key: string;
            }
          | {
              _key: string;
              _type: "downloadableFile";
              asset?: SanityFileAssetReference;
              media?: unknown;
              title?: string;
              label?: string;
              url: string | null;
            }
          | {
              _key: string;
              _type: "horizontalRule";
              info?: string;
            }
          | {
              asset: {
                _id: string;
                url: string;
                altText: string | null;
                metadata: {
                  lqip: string | null;
                  dimensions: {
                    width: number;
                    height: number;
                    aspectRatio: number;
                  } | null;
                } | null;
              } | null;
              media?: unknown;
              hotspot: SanityImageHotspot | null;
              crop: SanityImageCrop | null;
              alt: string;
              caption: string | null;
              _type: "image";
              _key: string;
            }
          | {
              _key: string;
              _type: "projectShowcaseSection";
              heading: string;
              subtitle?: string;
              items: Array<{
                _key: string;
                name: string;
                description: string;
                cta: {
                  label: string | null;
                  linkType: "external" | "internal" | null;
                  slug: string | "contato" | "inicio" | null;
                  externalUrl: string | null;
                  openInNewTab: boolean | null;
                } | null;
              }>;
            }
          | {
              _key: string;
              _type: "videoFile";
              file: {
                url: string | null;
                mimeType: string | null;
              };
              posterImage: {
                _key: null;
                hotspot: SanityImageHotspot | null;
                crop: SanityImageCrop | null;
                alt: null;
                caption: null;
                asset: {
                  _id: string;
                  url: string;
                  altText: string | null;
                  metadata: {
                    lqip: string | null;
                    dimensions: {
                      width: number;
                      height: number;
                      aspectRatio: number;
                    } | null;
                  } | null;
                } | null;
              } | null;
              caption?: string;
              autoplay?: boolean;
            }
          | {
              _key: string;
              _type: "youtubeEmbed";
              url: string;
              caption: string | null;
            }
        >;
        background: "default" | "muted";
      }
    | {
        _key: string;
        _type: "textWithIllustration";
        heading?: string;
        text: string;
        cta: {
          label: string | null;
          linkType: "external" | "internal" | null;
          slug: string | "contato" | "inicio" | null;
          externalUrl: string | null;
          openInNewTab: boolean | null;
        } | null;
        image: {
          _key: null;
          hotspot: SanityImageHotspot | null;
          crop: SanityImageCrop | null;
          alt: string | null;
          caption: null;
          asset: {
            _id: string;
            url: string;
            altText: string | null;
            metadata: {
              lqip: string | null;
              dimensions: {
                width: number;
                height: number;
                aspectRatio: number;
              } | null;
            } | null;
          } | null;
        } | null;
        imagePosition: "left" | "right";
      }
    | {
        _key: string;
        _type: "videoFile";
        file: {
          url: string | null;
          mimeType: string | null;
        };
        posterImage: {
          _key: null;
          hotspot: SanityImageHotspot | null;
          crop: SanityImageCrop | null;
          alt: null;
          caption: null;
          asset: {
            _id: string;
            url: string;
            altText: string | null;
            metadata: {
              lqip: string | null;
              dimensions: {
                width: number;
                height: number;
                aspectRatio: number;
              } | null;
            } | null;
          } | null;
        } | null;
        caption?: string;
        autoplay?: boolean;
      }
    | {
        _key: string;
        _type: "youtubeEmbed";
        url: string;
        caption: string | null;
      }
  > | null;
}>;

// Source: ../astro-imaa/src/lib/queries/page.ts
// Variable: PAGE_BY_SLUG_QUERY
// Query: *[_type == "page" && defined(slug.current) && slug.current == $slug][0] {        _id,  title,  excerpt,    "slug": slug.current,    hero {    heading,    tagline,  },  "featuredImage": featuredImage {   _key,  hotspot,  crop,  alt,  caption,  asset-> {    _id,    url,    altText,    metadata {      lqip,      dimensions {        width,        height,        aspectRatio      }    }  }, showInPage },  "pageBuilder": pageBuilder[] {   _key,  _type,  _type == "ctaSection" => {    ...,    "primaryCta": primaryCta {   label,  linkType,  "slug": select(  pageReference->_type == "homePage" => "inicio",  pageReference->_type == "contactPage" => "contato",  pageReference->slug.current),  externalUrl,  openInNewTab },    "secondaryCta": secondaryCta {   label,  linkType,  "slug": select(  pageReference->_type == "homePage" => "inicio",  pageReference->_type == "contactPage" => "contato",  pageReference->slug.current),  externalUrl,  openInNewTab }  },  _type == "textWithIllustration" => {    ...,    "image": image {   _key,  hotspot,  crop,  alt,  caption,  asset-> {    _id,    url,    altText,    metadata {      lqip,      dimensions {        width,        height,        aspectRatio      }    }  } },    "cta": cta {   label,  linkType,  "slug": select(  pageReference->_type == "homePage" => "inicio",  pageReference->_type == "contactPage" => "contato",  pageReference->slug.current),  externalUrl,  openInNewTab }  },  _type == "gallery" => {    ...,    "images": images[] {   _key,  hotspot,  crop,  alt,  caption,  asset-> {    _id,    url,    altText,    metadata {      lqip,      dimensions {        width,        height,        aspectRatio      }    }  } },    autoplay,    "autoplayInterval": coalesce(autoplayInterval, 4)  },  _type == "videoFile" => {   ...,  "file": {    "url": file.asset->url,    "mimeType": file.asset->mimeType  },  "posterImage": posterImage {   _key,  hotspot,  crop,  alt,  caption,  asset-> {    _id,    url,    altText,    metadata {      lqip,      dimensions {        width,        height,        aspectRatio      }    }  } } },  _type == "youtubeEmbed" => {   ...,  url,  caption },  _type == "horizontalRule" => {    ...  },  _type == "richText" => {    ...,    content[] {   ...,  _type == "image" => {   _key,  hotspot,  crop,  alt,  caption,  asset-> {    _id,    url,    altText,    metadata {      lqip,      dimensions {        width,        height,        aspectRatio      }    }  } },  _type == "videoFile" => {   ...,  "file": {    "url": file.asset->url,    "mimeType": file.asset->mimeType  },  "posterImage": posterImage {   _key,  hotspot,  crop,  alt,  caption,  asset-> {    _id,    url,    altText,    metadata {      lqip,      dimensions {        width,        height,        aspectRatio      }    }  } } },  _type == "youtubeEmbed" => {   ...,  url,  caption },  _type == "downloadableFile" => {   ...,  "url": asset->url },  _type == "projectShowcaseSection" => {   ...,  items[] {    _key,    name,    description,    "cta": cta {   label,  linkType,  "slug": select(  pageReference->_type == "homePage" => "inicio",  pageReference->_type == "contactPage" => "contato",  pageReference->slug.current),  externalUrl,  openInNewTab }  } }, },    background,  }, }  }
export type PAGE_BY_SLUG_QUERY_RESULT = {
  _id: string;
  title: string;
  excerpt: string;
  slug: string;
  hero: {
    heading: string | null;
    tagline: string | null;
  } | null;
  featuredImage: {
    _key: null;
    hotspot: SanityImageHotspot | null;
    crop: SanityImageCrop | null;
    alt: string | null;
    caption: null;
    asset: {
      _id: string;
      url: string;
      altText: string | null;
      metadata: {
        lqip: string | null;
        dimensions: {
          width: number;
          height: number;
          aspectRatio: number;
        } | null;
      } | null;
    } | null;
    showInPage: boolean | null;
  } | null;
  pageBuilder: Array<
    | {
        _key: string;
        _type: "ctaSection";
        heading: string;
        subheading?: string;
        primaryCta: {
          label: string | null;
          linkType: "external" | "internal" | null;
          slug: string | "contato" | "inicio" | null;
          externalUrl: string | null;
          openInNewTab: boolean | null;
        } | null;
        secondaryCta: {
          label: string | null;
          linkType: "external" | "internal" | null;
          slug: string | "contato" | "inicio" | null;
          externalUrl: string | null;
          openInNewTab: boolean | null;
        } | null;
        background: "muted" | "primary";
      }
    | {
        _key: string;
        _type: "gallery";
        heading?: string;
        layout: "carousel" | "grid2" | "grid3" | "masonry";
        autoplay: boolean | null;
        autoplayInterval: number | 4;
        images: Array<{
          _key: string;
          hotspot: SanityImageHotspot | null;
          crop: SanityImageCrop | null;
          alt: string | null;
          caption: string | null;
          asset: {
            _id: string;
            url: string;
            altText: string | null;
            metadata: {
              lqip: string | null;
              dimensions: {
                width: number;
                height: number;
                aspectRatio: number;
              } | null;
            } | null;
          } | null;
        }>;
      }
    | {
        _key: string;
        _type: "horizontalRule";
        info?: string;
      }
    | {
        _key: string;
        _type: "richText";
        content: Array<
          | {
              children?: Array<{
                marks?: Array<string>;
                text?: string;
                _type: "span";
                _key: string;
              }>;
              style?:
                | "blockquote"
                | "h2"
                | "h3"
                | "h4"
                | "h5"
                | "h6"
                | "normal";
              listItem?: "bullet" | "number";
              markDefs?: Array<{
                href: string;
                blank?: boolean;
                _type: "link";
                _key: string;
              }>;
              level?: number;
              _type: "block";
              _key: string;
            }
          | {
              _key: string;
              _type: "downloadableFile";
              asset?: SanityFileAssetReference;
              media?: unknown;
              title?: string;
              label?: string;
              url: string | null;
            }
          | {
              _key: string;
              _type: "horizontalRule";
              info?: string;
            }
          | {
              asset: {
                _id: string;
                url: string;
                altText: string | null;
                metadata: {
                  lqip: string | null;
                  dimensions: {
                    width: number;
                    height: number;
                    aspectRatio: number;
                  } | null;
                } | null;
              } | null;
              media?: unknown;
              hotspot: SanityImageHotspot | null;
              crop: SanityImageCrop | null;
              alt: string;
              caption: string | null;
              _type: "image";
              _key: string;
            }
          | {
              _key: string;
              _type: "projectShowcaseSection";
              heading: string;
              subtitle?: string;
              items: Array<{
                _key: string;
                name: string;
                description: string;
                cta: {
                  label: string | null;
                  linkType: "external" | "internal" | null;
                  slug: string | "contato" | "inicio" | null;
                  externalUrl: string | null;
                  openInNewTab: boolean | null;
                } | null;
              }>;
            }
          | {
              _key: string;
              _type: "videoFile";
              file: {
                url: string | null;
                mimeType: string | null;
              };
              posterImage: {
                _key: null;
                hotspot: SanityImageHotspot | null;
                crop: SanityImageCrop | null;
                alt: null;
                caption: null;
                asset: {
                  _id: string;
                  url: string;
                  altText: string | null;
                  metadata: {
                    lqip: string | null;
                    dimensions: {
                      width: number;
                      height: number;
                      aspectRatio: number;
                    } | null;
                  } | null;
                } | null;
              } | null;
              caption?: string;
              autoplay?: boolean;
            }
          | {
              _key: string;
              _type: "youtubeEmbed";
              url: string;
              caption: string | null;
            }
        >;
        background: "default" | "muted";
      }
    | {
        _key: string;
        _type: "textWithIllustration";
        heading?: string;
        text: string;
        cta: {
          label: string | null;
          linkType: "external" | "internal" | null;
          slug: string | "contato" | "inicio" | null;
          externalUrl: string | null;
          openInNewTab: boolean | null;
        } | null;
        image: {
          _key: null;
          hotspot: SanityImageHotspot | null;
          crop: SanityImageCrop | null;
          alt: string | null;
          caption: null;
          asset: {
            _id: string;
            url: string;
            altText: string | null;
            metadata: {
              lqip: string | null;
              dimensions: {
                width: number;
                height: number;
                aspectRatio: number;
              } | null;
            } | null;
          } | null;
        } | null;
        imagePosition: "left" | "right";
      }
    | {
        _key: string;
        _type: "videoFile";
        file: {
          url: string | null;
          mimeType: string | null;
        };
        posterImage: {
          _key: null;
          hotspot: SanityImageHotspot | null;
          crop: SanityImageCrop | null;
          alt: null;
          caption: null;
          asset: {
            _id: string;
            url: string;
            altText: string | null;
            metadata: {
              lqip: string | null;
              dimensions: {
                width: number;
                height: number;
                aspectRatio: number;
              } | null;
            } | null;
          } | null;
        } | null;
        caption?: string;
        autoplay?: boolean;
      }
    | {
        _key: string;
        _type: "youtubeEmbed";
        url: string;
        caption: string | null;
      }
  > | null;
} | null;

// Source: ../astro-imaa/src/lib/queries/site-settings.ts
// Variable: SITE_SETTINGS_QUERY
// Query: *[_id == "siteSettings"][0] {    title,    defaultSeoDescription,    "logo": logo {        _key,  hotspot,  crop,  alt,  caption,  asset-> {    _id,    url,    altText,    metadata {      lqip,      dimensions {        width,        height,        aspectRatio      }    }  },    },    "mainMenu": {      "label": coalesce(mainMenu.label, "Menu Principal"),      "items": coalesce(mainMenu.items[] {   _key,  "label": coalesce(label, pageReference->title),  "slug": select(    pageReference->_type == "homePage" => "inicio",    pageReference->_type == "contactPage" => "contato",    pageReference->slug.current  ),  "isDropdown": count(submenu) > 0,  "submenu": coalesce(submenu[] {    _key,    "label": coalesce(label, pageReference->title),    "slug": select(  pageReference->_type == "homePage" => "inicio",  pageReference->_type == "contactPage" => "contato",  pageReference->slug.current),  }, []), }, [])    },    "footerMenu": {      "label": coalesce(footerMenu.label, "Navegação"),      "items": coalesce(footerMenu.items[] {   _key,  "label": coalesce(label, pageReference->title),  "slug": select(    pageReference->_type == "homePage" => "inicio",    pageReference->_type == "contactPage" => "contato",    pageReference->slug.current  ),  "isDropdown": count(submenu) > 0,  "submenu": coalesce(submenu[] {    _key,    "label": coalesce(label, pageReference->title),    "slug": select(  pageReference->_type == "homePage" => "inicio",  pageReference->_type == "contactPage" => "contato",  pageReference->slug.current),  }, []), }, [])    },    "socialLinks": coalesce(socialLinks[] {      platform,      url,      label,      "image": image {          _key,  hotspot,  crop,  alt,  caption,  asset-> {    _id,    url,    altText,    metadata {      lqip,      dimensions {        width,        height,        aspectRatio      }    }  }      }    }, []),    "contactInfo": coalesce(contactInfo {      address,      phones,      emails    }, { "address": null, "phones": [], "emails": [] })  }
export type SITE_SETTINGS_QUERY_RESULT =
  | {
      title: string;
      defaultSeoDescription: string | null;
      logo: {
        _key: null;
        hotspot: SanityImageHotspot | null;
        crop: SanityImageCrop | null;
        alt: string | null;
        caption: null;
        asset: {
          _id: string;
          url: string;
          altText: string | null;
          metadata: {
            lqip: string | null;
            dimensions: {
              width: number;
              height: number;
              aspectRatio: number;
            } | null;
          } | null;
        } | null;
      } | null;
      mainMenu: {
        label: string | "Menu Principal";
        items:
          | Array<{
              _key: string;
              label: string | null;
              slug: string | "contato" | "inicio" | null;
              isDropdown: boolean | null;
              submenu:
                | Array<{
                    _key: string;
                    label: string | null;
                    slug: string | "contato" | "inicio" | null;
                  }>
                | Array<never>;
            }>
          | Array<never>;
      };
      footerMenu: {
        label: string | "Navega\xE7\xE3o";
        items:
          | Array<{
              _key: string;
              label: string | null;
              slug: string | "contato" | "inicio" | null;
              isDropdown: boolean | null;
              submenu:
                | Array<{
                    _key: string;
                    label: string | null;
                    slug: string | "contato" | "inicio" | null;
                  }>
                | Array<never>;
            }>
          | Array<never>;
      };
      socialLinks:
        | Array<{
            platform:
              | "bluesky"
              | "facebook"
              | "instagram"
              | "twitter"
              | "youtube";
            url: string;
            label: string | null;
            image: {
              _key: null;
              hotspot: SanityImageHotspot | null;
              crop: SanityImageCrop | null;
              alt: string | null;
              caption: null;
              asset: {
                _id: string;
                url: string;
                altText: string | null;
                metadata: {
                  lqip: string | null;
                  dimensions: {
                    width: number;
                    height: number;
                    aspectRatio: number;
                  } | null;
                } | null;
              } | null;
            } | null;
          }>
        | Array<never>;
      contactInfo:
        | {
            address: null;
            phones: Array<never>;
            emails: Array<never>;
          }
        | {
            address: Address | null;
            phones: Array<{
              label?: string;
              number: string;
              isWhatsApp?: boolean;
              _key: string;
            }> | null;
            emails: Array<{
              label?: string;
              address: string;
              _key: string;
            }> | null;
          };
    }
  | {
      title: null;
      defaultSeoDescription: null;
      logo: null;
      mainMenu: {
        label: "Menu Principal";
        items: Array<never>;
      };
      footerMenu: {
        label: "Navega\xE7\xE3o";
        items: Array<never>;
      };
      socialLinks: Array<never>;
      contactInfo: {
        address: null;
        phones: Array<never>;
        emails: Array<never>;
      };
    }
  | {
      title: string;
      defaultSeoDescription: null;
      logo: null;
      mainMenu: {
        label: "Menu Principal";
        items: Array<never>;
      };
      footerMenu: {
        label: "Navega\xE7\xE3o";
        items: Array<never>;
      };
      socialLinks: Array<never>;
      contactInfo: {
        address: null;
        phones: Array<never>;
        emails: Array<never>;
      };
    }
  | {
      title: string | null;
      defaultSeoDescription: null;
      logo: null;
      mainMenu: {
        label: "Menu Principal";
        items: Array<never>;
      };
      footerMenu: {
        label: "Navega\xE7\xE3o";
        items: Array<never>;
      };
      socialLinks: Array<never>;
      contactInfo: {
        address: null;
        phones: Array<never>;
        emails: Array<never>;
      };
    }
  | {
      title: string;
      defaultSeoDescription: null;
      logo: {
        _key: null;
        hotspot: SanityImageHotspot | null;
        crop: SanityImageCrop | null;
        alt: string | null;
        caption: null;
        asset: {
          _id: string;
          url: string;
          altText: string | null;
          metadata: {
            lqip: string | null;
            dimensions: {
              width: number;
              height: number;
              aspectRatio: number;
            } | null;
          } | null;
        } | null;
      } | null;
      mainMenu: {
        label: "Menu Principal";
        items: Array<never>;
      };
      footerMenu: {
        label: "Navega\xE7\xE3o";
        items: Array<never>;
      };
      socialLinks: Array<never>;
      contactInfo: {
        address: null;
        phones: Array<never>;
        emails: Array<never>;
      };
    }
  | null;

// Source: ../astro-imaa/src/lib/queries/transparency.ts
// Variable: TRANSPARENCY_INDEX_PAGE_QUERY
// Query: *[_type == "page" && slug.current == "transparencia"][0] {        _id,  title,  excerpt,    "slug": slug.current,    hero {    heading,    tagline,  },  "featuredImage": featuredImage {   _key,  hotspot,  crop,  alt,  caption,  asset-> {    _id,    url,    altText,    metadata {      lqip,      dimensions {        width,        height,        aspectRatio      }    }  }, showInPage },  "pageBuilder": pageBuilder[] {   _key,  _type,  _type == "ctaSection" => {    ...,    "primaryCta": primaryCta {   label,  linkType,  "slug": select(  pageReference->_type == "homePage" => "inicio",  pageReference->_type == "contactPage" => "contato",  pageReference->slug.current),  externalUrl,  openInNewTab },    "secondaryCta": secondaryCta {   label,  linkType,  "slug": select(  pageReference->_type == "homePage" => "inicio",  pageReference->_type == "contactPage" => "contato",  pageReference->slug.current),  externalUrl,  openInNewTab }  },  _type == "textWithIllustration" => {    ...,    "image": image {   _key,  hotspot,  crop,  alt,  caption,  asset-> {    _id,    url,    altText,    metadata {      lqip,      dimensions {        width,        height,        aspectRatio      }    }  } },    "cta": cta {   label,  linkType,  "slug": select(  pageReference->_type == "homePage" => "inicio",  pageReference->_type == "contactPage" => "contato",  pageReference->slug.current),  externalUrl,  openInNewTab }  },  _type == "gallery" => {    ...,    "images": images[] {   _key,  hotspot,  crop,  alt,  caption,  asset-> {    _id,    url,    altText,    metadata {      lqip,      dimensions {        width,        height,        aspectRatio      }    }  } },    autoplay,    "autoplayInterval": coalesce(autoplayInterval, 4)  },  _type == "videoFile" => {   ...,  "file": {    "url": file.asset->url,    "mimeType": file.asset->mimeType  },  "posterImage": posterImage {   _key,  hotspot,  crop,  alt,  caption,  asset-> {    _id,    url,    altText,    metadata {      lqip,      dimensions {        width,        height,        aspectRatio      }    }  } } },  _type == "youtubeEmbed" => {   ...,  url,  caption },  _type == "horizontalRule" => {    ...  },  _type == "richText" => {    ...,    content[] {   ...,  _type == "image" => {   _key,  hotspot,  crop,  alt,  caption,  asset-> {    _id,    url,    altText,    metadata {      lqip,      dimensions {        width,        height,        aspectRatio      }    }  } },  _type == "videoFile" => {   ...,  "file": {    "url": file.asset->url,    "mimeType": file.asset->mimeType  },  "posterImage": posterImage {   _key,  hotspot,  crop,  alt,  caption,  asset-> {    _id,    url,    altText,    metadata {      lqip,      dimensions {        width,        height,        aspectRatio      }    }  } } },  _type == "youtubeEmbed" => {   ...,  url,  caption },  _type == "downloadableFile" => {   ...,  "url": asset->url },  _type == "projectShowcaseSection" => {   ...,  items[] {    _key,    name,    description,    "cta": cta {   label,  linkType,  "slug": select(  pageReference->_type == "homePage" => "inicio",  pageReference->_type == "contactPage" => "contato",  pageReference->slug.current),  externalUrl,  openInNewTab }  } }, },    background,  }, }  }
export type TRANSPARENCY_INDEX_PAGE_QUERY_RESULT = {
  _id: string;
  title: string;
  excerpt: string;
  slug: string;
  hero: {
    heading: string | null;
    tagline: string | null;
  } | null;
  featuredImage: {
    _key: null;
    hotspot: SanityImageHotspot | null;
    crop: SanityImageCrop | null;
    alt: string | null;
    caption: null;
    asset: {
      _id: string;
      url: string;
      altText: string | null;
      metadata: {
        lqip: string | null;
        dimensions: {
          width: number;
          height: number;
          aspectRatio: number;
        } | null;
      } | null;
    } | null;
    showInPage: boolean | null;
  } | null;
  pageBuilder: Array<
    | {
        _key: string;
        _type: "ctaSection";
        heading: string;
        subheading?: string;
        primaryCta: {
          label: string | null;
          linkType: "external" | "internal" | null;
          slug: string | "contato" | "inicio" | null;
          externalUrl: string | null;
          openInNewTab: boolean | null;
        } | null;
        secondaryCta: {
          label: string | null;
          linkType: "external" | "internal" | null;
          slug: string | "contato" | "inicio" | null;
          externalUrl: string | null;
          openInNewTab: boolean | null;
        } | null;
        background: "muted" | "primary";
      }
    | {
        _key: string;
        _type: "gallery";
        heading?: string;
        layout: "carousel" | "grid2" | "grid3" | "masonry";
        autoplay: boolean | null;
        autoplayInterval: number | 4;
        images: Array<{
          _key: string;
          hotspot: SanityImageHotspot | null;
          crop: SanityImageCrop | null;
          alt: string | null;
          caption: string | null;
          asset: {
            _id: string;
            url: string;
            altText: string | null;
            metadata: {
              lqip: string | null;
              dimensions: {
                width: number;
                height: number;
                aspectRatio: number;
              } | null;
            } | null;
          } | null;
        }>;
      }
    | {
        _key: string;
        _type: "horizontalRule";
        info?: string;
      }
    | {
        _key: string;
        _type: "richText";
        content: Array<
          | {
              children?: Array<{
                marks?: Array<string>;
                text?: string;
                _type: "span";
                _key: string;
              }>;
              style?:
                | "blockquote"
                | "h2"
                | "h3"
                | "h4"
                | "h5"
                | "h6"
                | "normal";
              listItem?: "bullet" | "number";
              markDefs?: Array<{
                href: string;
                blank?: boolean;
                _type: "link";
                _key: string;
              }>;
              level?: number;
              _type: "block";
              _key: string;
            }
          | {
              _key: string;
              _type: "downloadableFile";
              asset?: SanityFileAssetReference;
              media?: unknown;
              title?: string;
              label?: string;
              url: string | null;
            }
          | {
              _key: string;
              _type: "horizontalRule";
              info?: string;
            }
          | {
              asset: {
                _id: string;
                url: string;
                altText: string | null;
                metadata: {
                  lqip: string | null;
                  dimensions: {
                    width: number;
                    height: number;
                    aspectRatio: number;
                  } | null;
                } | null;
              } | null;
              media?: unknown;
              hotspot: SanityImageHotspot | null;
              crop: SanityImageCrop | null;
              alt: string;
              caption: string | null;
              _type: "image";
              _key: string;
            }
          | {
              _key: string;
              _type: "projectShowcaseSection";
              heading: string;
              subtitle?: string;
              items: Array<{
                _key: string;
                name: string;
                description: string;
                cta: {
                  label: string | null;
                  linkType: "external" | "internal" | null;
                  slug: string | "contato" | "inicio" | null;
                  externalUrl: string | null;
                  openInNewTab: boolean | null;
                } | null;
              }>;
            }
          | {
              _key: string;
              _type: "videoFile";
              file: {
                url: string | null;
                mimeType: string | null;
              };
              posterImage: {
                _key: null;
                hotspot: SanityImageHotspot | null;
                crop: SanityImageCrop | null;
                alt: null;
                caption: null;
                asset: {
                  _id: string;
                  url: string;
                  altText: string | null;
                  metadata: {
                    lqip: string | null;
                    dimensions: {
                      width: number;
                      height: number;
                      aspectRatio: number;
                    } | null;
                  } | null;
                } | null;
              } | null;
              caption?: string;
              autoplay?: boolean;
            }
          | {
              _key: string;
              _type: "youtubeEmbed";
              url: string;
              caption: string | null;
            }
        >;
        background: "default" | "muted";
      }
    | {
        _key: string;
        _type: "textWithIllustration";
        heading?: string;
        text: string;
        cta: {
          label: string | null;
          linkType: "external" | "internal" | null;
          slug: string | "contato" | "inicio" | null;
          externalUrl: string | null;
          openInNewTab: boolean | null;
        } | null;
        image: {
          _key: null;
          hotspot: SanityImageHotspot | null;
          crop: SanityImageCrop | null;
          alt: string | null;
          caption: null;
          asset: {
            _id: string;
            url: string;
            altText: string | null;
            metadata: {
              lqip: string | null;
              dimensions: {
                width: number;
                height: number;
                aspectRatio: number;
              } | null;
            } | null;
          } | null;
        } | null;
        imagePosition: "left" | "right";
      }
    | {
        _key: string;
        _type: "videoFile";
        file: {
          url: string | null;
          mimeType: string | null;
        };
        posterImage: {
          _key: null;
          hotspot: SanityImageHotspot | null;
          crop: SanityImageCrop | null;
          alt: null;
          caption: null;
          asset: {
            _id: string;
            url: string;
            altText: string | null;
            metadata: {
              lqip: string | null;
              dimensions: {
                width: number;
                height: number;
                aspectRatio: number;
              } | null;
            } | null;
          } | null;
        } | null;
        caption?: string;
        autoplay?: boolean;
      }
    | {
        _key: string;
        _type: "youtubeEmbed";
        url: string;
        caption: string | null;
      }
  > | null;
} | null;

// Source: ../astro-imaa/src/lib/queries/transparency.ts
// Variable: ALL_TRANSPARENCY_SECTIONS_QUERY
// Query: *[_type == "transparencySection"] | order(order asc) {      _id,  title,    "slug": slug.current,    hero {    heading,    tagline,  },  partner,  excerpt,  description[] {   ...,  _type == "image" => {   _key,  hotspot,  crop,  alt,  caption,  asset-> {    _id,    url,    altText,    metadata {      lqip,      dimensions {        width,        height,        aspectRatio      }    }  } },  _type == "videoFile" => {   ...,  "file": {    "url": file.asset->url,    "mimeType": file.asset->mimeType  },  "posterImage": posterImage {   _key,  hotspot,  crop,  alt,  caption,  asset-> {    _id,    url,    altText,    metadata {      lqip,      dimensions {        width,        height,        aspectRatio      }    }  } } },  _type == "youtubeEmbed" => {   ...,  url,  caption },  _type == "downloadableFile" => {   ...,  "url": asset->url },  _type == "projectShowcaseSection" => {   ...,  items[] {    _key,    name,    description,    "cta": cta {   label,  linkType,  "slug": select(  pageReference->_type == "homePage" => "inicio",  pageReference->_type == "contactPage" => "contato",  pageReference->slug.current),  externalUrl,  openInNewTab }  } }, },  projects[] {   _key,  title,  origin,  value,  status,  documents[] {   _key,  title,  label,  "fileUrl": asset->url,  "fileName": asset->originalFilename, }, },  documents[] {   _key,  title,  label,  "fileUrl": asset->url,  "fileName": asset->originalFilename, },  }
export type ALL_TRANSPARENCY_SECTIONS_QUERY_RESULT = Array<{
  _id: string;
  title: string;
  slug: string;
  hero: {
    heading: string | null;
    tagline: string | null;
  } | null;
  partner: string;
  excerpt: string;
  description: Array<
    | {
        children?: Array<{
          marks?: Array<string>;
          text?: string;
          _type: "span";
          _key: string;
        }>;
        style?: "blockquote" | "h2" | "h3" | "h4" | "h5" | "h6" | "normal";
        listItem?: "bullet" | "number";
        markDefs?: Array<{
          href: string;
          blank?: boolean;
          _type: "link";
          _key: string;
        }>;
        level?: number;
        _type: "block";
        _key: string;
      }
    | {
        _key: string;
        _type: "downloadableFile";
        asset?: SanityFileAssetReference;
        media?: unknown;
        title?: string;
        label?: string;
        url: string | null;
      }
    | {
        _key: string;
        _type: "horizontalRule";
        info?: string;
      }
    | {
        asset: {
          _id: string;
          url: string;
          altText: string | null;
          metadata: {
            lqip: string | null;
            dimensions: {
              width: number;
              height: number;
              aspectRatio: number;
            } | null;
          } | null;
        } | null;
        media?: unknown;
        hotspot: SanityImageHotspot | null;
        crop: SanityImageCrop | null;
        alt: string;
        caption: string | null;
        _type: "image";
        _key: string;
      }
    | {
        _key: string;
        _type: "projectShowcaseSection";
        heading: string;
        subtitle?: string;
        items: Array<{
          _key: string;
          name: string;
          description: string;
          cta: {
            label: string | null;
            linkType: "external" | "internal" | null;
            slug: string | "contato" | "inicio" | null;
            externalUrl: string | null;
            openInNewTab: boolean | null;
          } | null;
        }>;
      }
    | {
        _key: string;
        _type: "videoFile";
        file: {
          url: string | null;
          mimeType: string | null;
        };
        posterImage: {
          _key: null;
          hotspot: SanityImageHotspot | null;
          crop: SanityImageCrop | null;
          alt: null;
          caption: null;
          asset: {
            _id: string;
            url: string;
            altText: string | null;
            metadata: {
              lqip: string | null;
              dimensions: {
                width: number;
                height: number;
                aspectRatio: number;
              } | null;
            } | null;
          } | null;
        } | null;
        caption?: string;
        autoplay?: boolean;
      }
    | {
        _key: string;
        _type: "youtubeEmbed";
        url: string;
        caption: string | null;
      }
  > | null;
  projects: Array<{
    _key: string;
    title: string;
    origin: string | null;
    value: string | null;
    status: "completed" | "in_analysis" | "in_progress" | "pending" | null;
    documents: Array<{
      _key: string;
      title: string | null;
      label: string | null;
      fileUrl: string | null;
      fileName: string | null;
    }> | null;
  }> | null;
  documents: Array<{
    _key: string;
    title: string | null;
    label: string | null;
    fileUrl: string | null;
    fileName: string | null;
  }> | null;
}>;

// Source: ../astro-imaa/src/lib/queries/transparency.ts
// Variable: TRANSPARENCY_SECTION_BY_SLUG_QUERY
// Query: *[_type == "transparencySection" && slug.current == $slug][0] {      _id,  title,    "slug": slug.current,    hero {    heading,    tagline,  },  partner,  excerpt,  description[] {   ...,  _type == "image" => {   _key,  hotspot,  crop,  alt,  caption,  asset-> {    _id,    url,    altText,    metadata {      lqip,      dimensions {        width,        height,        aspectRatio      }    }  } },  _type == "videoFile" => {   ...,  "file": {    "url": file.asset->url,    "mimeType": file.asset->mimeType  },  "posterImage": posterImage {   _key,  hotspot,  crop,  alt,  caption,  asset-> {    _id,    url,    altText,    metadata {      lqip,      dimensions {        width,        height,        aspectRatio      }    }  } } },  _type == "youtubeEmbed" => {   ...,  url,  caption },  _type == "downloadableFile" => {   ...,  "url": asset->url },  _type == "projectShowcaseSection" => {   ...,  items[] {    _key,    name,    description,    "cta": cta {   label,  linkType,  "slug": select(  pageReference->_type == "homePage" => "inicio",  pageReference->_type == "contactPage" => "contato",  pageReference->slug.current),  externalUrl,  openInNewTab }  } }, },  projects[] {   _key,  title,  origin,  value,  status,  documents[] {   _key,  title,  label,  "fileUrl": asset->url,  "fileName": asset->originalFilename, }, },  documents[] {   _key,  title,  label,  "fileUrl": asset->url,  "fileName": asset->originalFilename, },  }
export type TRANSPARENCY_SECTION_BY_SLUG_QUERY_RESULT = {
  _id: string;
  title: string;
  slug: string;
  hero: {
    heading: string | null;
    tagline: string | null;
  } | null;
  partner: string;
  excerpt: string;
  description: Array<
    | {
        children?: Array<{
          marks?: Array<string>;
          text?: string;
          _type: "span";
          _key: string;
        }>;
        style?: "blockquote" | "h2" | "h3" | "h4" | "h5" | "h6" | "normal";
        listItem?: "bullet" | "number";
        markDefs?: Array<{
          href: string;
          blank?: boolean;
          _type: "link";
          _key: string;
        }>;
        level?: number;
        _type: "block";
        _key: string;
      }
    | {
        _key: string;
        _type: "downloadableFile";
        asset?: SanityFileAssetReference;
        media?: unknown;
        title?: string;
        label?: string;
        url: string | null;
      }
    | {
        _key: string;
        _type: "horizontalRule";
        info?: string;
      }
    | {
        asset: {
          _id: string;
          url: string;
          altText: string | null;
          metadata: {
            lqip: string | null;
            dimensions: {
              width: number;
              height: number;
              aspectRatio: number;
            } | null;
          } | null;
        } | null;
        media?: unknown;
        hotspot: SanityImageHotspot | null;
        crop: SanityImageCrop | null;
        alt: string;
        caption: string | null;
        _type: "image";
        _key: string;
      }
    | {
        _key: string;
        _type: "projectShowcaseSection";
        heading: string;
        subtitle?: string;
        items: Array<{
          _key: string;
          name: string;
          description: string;
          cta: {
            label: string | null;
            linkType: "external" | "internal" | null;
            slug: string | "contato" | "inicio" | null;
            externalUrl: string | null;
            openInNewTab: boolean | null;
          } | null;
        }>;
      }
    | {
        _key: string;
        _type: "videoFile";
        file: {
          url: string | null;
          mimeType: string | null;
        };
        posterImage: {
          _key: null;
          hotspot: SanityImageHotspot | null;
          crop: SanityImageCrop | null;
          alt: null;
          caption: null;
          asset: {
            _id: string;
            url: string;
            altText: string | null;
            metadata: {
              lqip: string | null;
              dimensions: {
                width: number;
                height: number;
                aspectRatio: number;
              } | null;
            } | null;
          } | null;
        } | null;
        caption?: string;
        autoplay?: boolean;
      }
    | {
        _key: string;
        _type: "youtubeEmbed";
        url: string;
        caption: string | null;
      }
  > | null;
  projects: Array<{
    _key: string;
    title: string;
    origin: string | null;
    value: string | null;
    status: "completed" | "in_analysis" | "in_progress" | "pending" | null;
    documents: Array<{
      _key: string;
      title: string | null;
      label: string | null;
      fileUrl: string | null;
      fileName: string | null;
    }> | null;
  }> | null;
  documents: Array<{
    _key: string;
    title: string | null;
    label: string | null;
    fileUrl: string | null;
    fileName: string | null;
  }> | null;
} | null;

// Query TypeMap
import "@sanity/client";
declare module "@sanity/client" {
  interface SanityQueries {
    '\n  *[_type == "contactPage"][0] {\n    heading,\n    tagline,\n    seoDescription,\n    contactTabLabel,\n    contactFormHeading,\n    contactFormDescription,\n    enrollmentActive,\n    enrollmentTabLabel,\n    enrollmentFormHeading,\n    enrollmentFormDescription,\n    "enrollmentFormFields": enrollmentFormFields[] {\n      _key,\n      fieldType,\n      label,\n      placeholder,\n      inputType,\n      required,\n      width,\n      "options": options[] {\n        _key,\n        label\n      }\n    }\n  }\n': CONTACT_PAGE_QUERY_RESULT;
    '\n  *[_id == "homePage"][0] {\n    title,\n    excerpt,\n    heroDescription,\n    "logo": logo {\n      \n  _key,\n  hotspot,\n  crop,\n  alt,\n  caption,\n  asset-> {\n    _id,\n    url,\n    altText,\n    metadata {\n      lqip,\n      dimensions {\n        width,\n        height,\n        aspectRatio\n      }\n    }\n  }\n\n    },\n    "primaryCta": primaryCta { \n  label,\n  linkType,\n  "slug": select(\n  pageReference->_type == "homePage" => "inicio",\n  pageReference->_type == "contactPage" => "contato",\n  pageReference->slug.current\n),\n  externalUrl,\n  openInNewTab\n },\n    "secondaryCta": secondaryCta { \n  label,\n  linkType,\n  "slug": select(\n  pageReference->_type == "homePage" => "inicio",\n  pageReference->_type == "contactPage" => "contato",\n  pageReference->slug.current\n),\n  externalUrl,\n  openInNewTab\n },\n    "projectsPreview": projectsPreview {\n      label,\n      heading,\n      "image": image { \n  _key,\n  hotspot,\n  crop,\n  alt,\n  caption,\n  asset-> {\n    _id,\n    url,\n    altText,\n    metadata {\n      lqip,\n      dimensions {\n        width,\n        height,\n        aspectRatio\n      }\n    }\n  }\n },\n      projects[] {\n        _key,\n        title,\n        description,\n        accent\n      },\n    },\n    "missionSection": missionSection {\n      label,\n      heading,\n      headingHighlight,\n      description,\n      secondaryDescription,\n      "image": image { \n  _key,\n  hotspot,\n  crop,\n  alt,\n  caption,\n  asset-> {\n    _id,\n    url,\n    altText,\n    metadata {\n      lqip,\n      dimensions {\n        width,\n        height,\n        aspectRatio\n      }\n    }\n  }\n },\n      pillars[] {\n        _key,\n        icon,\n        title,\n        description,\n        color\n      },\n    },\n    "blockHighlightSection": blockHighlightSection {\n      label,\n      milestone,\n      milestoneLabel,\n      heading,\n      body,\n      footer,\n    },\n  }\n': HOME_PAGE_QUERY_RESULT;
    '\n  *[_type == "page" && defined(slug.current)] | order(_createdAt desc) {\n    \n  \n  _id,\n  title,\n  excerpt,\n  \n  "slug": slug.current\n\n,\n  \n  hero {\n    heading,\n    tagline,\n  }\n,\n  "featuredImage": featuredImage { \n  _key,\n  hotspot,\n  crop,\n  alt,\n  caption,\n  asset-> {\n    _id,\n    url,\n    altText,\n    metadata {\n      lqip,\n      dimensions {\n        width,\n        height,\n        aspectRatio\n      }\n    }\n  }\n, showInPage },\n  "pageBuilder": pageBuilder[] { \n  _key,\n  _type,\n  _type == "ctaSection" => {\n    ...,\n    "primaryCta": primaryCta { \n  label,\n  linkType,\n  "slug": select(\n  pageReference->_type == "homePage" => "inicio",\n  pageReference->_type == "contactPage" => "contato",\n  pageReference->slug.current\n),\n  externalUrl,\n  openInNewTab\n },\n    "secondaryCta": secondaryCta { \n  label,\n  linkType,\n  "slug": select(\n  pageReference->_type == "homePage" => "inicio",\n  pageReference->_type == "contactPage" => "contato",\n  pageReference->slug.current\n),\n  externalUrl,\n  openInNewTab\n }\n  },\n  _type == "textWithIllustration" => {\n    ...,\n    "image": image { \n  _key,\n  hotspot,\n  crop,\n  alt,\n  caption,\n  asset-> {\n    _id,\n    url,\n    altText,\n    metadata {\n      lqip,\n      dimensions {\n        width,\n        height,\n        aspectRatio\n      }\n    }\n  }\n },\n    "cta": cta { \n  label,\n  linkType,\n  "slug": select(\n  pageReference->_type == "homePage" => "inicio",\n  pageReference->_type == "contactPage" => "contato",\n  pageReference->slug.current\n),\n  externalUrl,\n  openInNewTab\n }\n  },\n  _type == "gallery" => {\n    ...,\n    "images": images[] { \n  _key,\n  hotspot,\n  crop,\n  alt,\n  caption,\n  asset-> {\n    _id,\n    url,\n    altText,\n    metadata {\n      lqip,\n      dimensions {\n        width,\n        height,\n        aspectRatio\n      }\n    }\n  }\n },\n    autoplay,\n    "autoplayInterval": coalesce(autoplayInterval, 4)\n  },\n  _type == "videoFile" => { \n  ...,\n  "file": {\n    "url": file.asset->url,\n    "mimeType": file.asset->mimeType\n  },\n  "posterImage": posterImage { \n  _key,\n  hotspot,\n  crop,\n  alt,\n  caption,\n  asset-> {\n    _id,\n    url,\n    altText,\n    metadata {\n      lqip,\n      dimensions {\n        width,\n        height,\n        aspectRatio\n      }\n    }\n  }\n }\n },\n  _type == "youtubeEmbed" => { \n  ...,\n  url,\n  caption\n },\n  _type == "horizontalRule" => {\n    ...\n  },\n  _type == "richText" => {\n    ...,\n    content[] { \n  ...,\n  _type == "image" => { \n  _key,\n  hotspot,\n  crop,\n  alt,\n  caption,\n  asset-> {\n    _id,\n    url,\n    altText,\n    metadata {\n      lqip,\n      dimensions {\n        width,\n        height,\n        aspectRatio\n      }\n    }\n  }\n },\n  _type == "videoFile" => { \n  ...,\n  "file": {\n    "url": file.asset->url,\n    "mimeType": file.asset->mimeType\n  },\n  "posterImage": posterImage { \n  _key,\n  hotspot,\n  crop,\n  alt,\n  caption,\n  asset-> {\n    _id,\n    url,\n    altText,\n    metadata {\n      lqip,\n      dimensions {\n        width,\n        height,\n        aspectRatio\n      }\n    }\n  }\n }\n },\n  _type == "youtubeEmbed" => { \n  ...,\n  url,\n  caption\n },\n  _type == "downloadableFile" => { \n  ...,\n  "url": asset->url\n },\n  _type == "projectShowcaseSection" => { \n  ...,\n  items[] {\n    _key,\n    name,\n    description,\n    "cta": cta { \n  label,\n  linkType,\n  "slug": select(\n  pageReference->_type == "homePage" => "inicio",\n  pageReference->_type == "contactPage" => "contato",\n  pageReference->slug.current\n),\n  externalUrl,\n  openInNewTab\n }\n  }\n },\n },\n    background,\n  },\n }\n\n  }\n': ALL_PAGES_QUERY_RESULT;
    '\n  *[_type == "page" && defined(slug.current) && slug.current == $slug][0] {\n    \n  \n  _id,\n  title,\n  excerpt,\n  \n  "slug": slug.current\n\n,\n  \n  hero {\n    heading,\n    tagline,\n  }\n,\n  "featuredImage": featuredImage { \n  _key,\n  hotspot,\n  crop,\n  alt,\n  caption,\n  asset-> {\n    _id,\n    url,\n    altText,\n    metadata {\n      lqip,\n      dimensions {\n        width,\n        height,\n        aspectRatio\n      }\n    }\n  }\n, showInPage },\n  "pageBuilder": pageBuilder[] { \n  _key,\n  _type,\n  _type == "ctaSection" => {\n    ...,\n    "primaryCta": primaryCta { \n  label,\n  linkType,\n  "slug": select(\n  pageReference->_type == "homePage" => "inicio",\n  pageReference->_type == "contactPage" => "contato",\n  pageReference->slug.current\n),\n  externalUrl,\n  openInNewTab\n },\n    "secondaryCta": secondaryCta { \n  label,\n  linkType,\n  "slug": select(\n  pageReference->_type == "homePage" => "inicio",\n  pageReference->_type == "contactPage" => "contato",\n  pageReference->slug.current\n),\n  externalUrl,\n  openInNewTab\n }\n  },\n  _type == "textWithIllustration" => {\n    ...,\n    "image": image { \n  _key,\n  hotspot,\n  crop,\n  alt,\n  caption,\n  asset-> {\n    _id,\n    url,\n    altText,\n    metadata {\n      lqip,\n      dimensions {\n        width,\n        height,\n        aspectRatio\n      }\n    }\n  }\n },\n    "cta": cta { \n  label,\n  linkType,\n  "slug": select(\n  pageReference->_type == "homePage" => "inicio",\n  pageReference->_type == "contactPage" => "contato",\n  pageReference->slug.current\n),\n  externalUrl,\n  openInNewTab\n }\n  },\n  _type == "gallery" => {\n    ...,\n    "images": images[] { \n  _key,\n  hotspot,\n  crop,\n  alt,\n  caption,\n  asset-> {\n    _id,\n    url,\n    altText,\n    metadata {\n      lqip,\n      dimensions {\n        width,\n        height,\n        aspectRatio\n      }\n    }\n  }\n },\n    autoplay,\n    "autoplayInterval": coalesce(autoplayInterval, 4)\n  },\n  _type == "videoFile" => { \n  ...,\n  "file": {\n    "url": file.asset->url,\n    "mimeType": file.asset->mimeType\n  },\n  "posterImage": posterImage { \n  _key,\n  hotspot,\n  crop,\n  alt,\n  caption,\n  asset-> {\n    _id,\n    url,\n    altText,\n    metadata {\n      lqip,\n      dimensions {\n        width,\n        height,\n        aspectRatio\n      }\n    }\n  }\n }\n },\n  _type == "youtubeEmbed" => { \n  ...,\n  url,\n  caption\n },\n  _type == "horizontalRule" => {\n    ...\n  },\n  _type == "richText" => {\n    ...,\n    content[] { \n  ...,\n  _type == "image" => { \n  _key,\n  hotspot,\n  crop,\n  alt,\n  caption,\n  asset-> {\n    _id,\n    url,\n    altText,\n    metadata {\n      lqip,\n      dimensions {\n        width,\n        height,\n        aspectRatio\n      }\n    }\n  }\n },\n  _type == "videoFile" => { \n  ...,\n  "file": {\n    "url": file.asset->url,\n    "mimeType": file.asset->mimeType\n  },\n  "posterImage": posterImage { \n  _key,\n  hotspot,\n  crop,\n  alt,\n  caption,\n  asset-> {\n    _id,\n    url,\n    altText,\n    metadata {\n      lqip,\n      dimensions {\n        width,\n        height,\n        aspectRatio\n      }\n    }\n  }\n }\n },\n  _type == "youtubeEmbed" => { \n  ...,\n  url,\n  caption\n },\n  _type == "downloadableFile" => { \n  ...,\n  "url": asset->url\n },\n  _type == "projectShowcaseSection" => { \n  ...,\n  items[] {\n    _key,\n    name,\n    description,\n    "cta": cta { \n  label,\n  linkType,\n  "slug": select(\n  pageReference->_type == "homePage" => "inicio",\n  pageReference->_type == "contactPage" => "contato",\n  pageReference->slug.current\n),\n  externalUrl,\n  openInNewTab\n }\n  }\n },\n },\n    background,\n  },\n }\n\n  }\n': PAGE_BY_SLUG_QUERY_RESULT;
    '\n  *[_id == "siteSettings"][0] {\n    title,\n    defaultSeoDescription,\n    "logo": logo {\n      \n  _key,\n  hotspot,\n  crop,\n  alt,\n  caption,\n  asset-> {\n    _id,\n    url,\n    altText,\n    metadata {\n      lqip,\n      dimensions {\n        width,\n        height,\n        aspectRatio\n      }\n    }\n  }\n,\n    },\n    "mainMenu": {\n      "label": coalesce(mainMenu.label, "Menu Principal"),\n      "items": coalesce(mainMenu.items[] { \n  _key,\n  "label": coalesce(label, pageReference->title),\n  "slug": select(\n    pageReference->_type == "homePage" => "inicio",\n    pageReference->_type == "contactPage" => "contato",\n    pageReference->slug.current\n  ),\n  "isDropdown": count(submenu) > 0,\n  "submenu": coalesce(submenu[] {\n    _key,\n    "label": coalesce(label, pageReference->title),\n    "slug": select(\n  pageReference->_type == "homePage" => "inicio",\n  pageReference->_type == "contactPage" => "contato",\n  pageReference->slug.current\n),\n  }, []),\n }, [])\n    },\n    "footerMenu": {\n      "label": coalesce(footerMenu.label, "Navega\xE7\xE3o"),\n      "items": coalesce(footerMenu.items[] { \n  _key,\n  "label": coalesce(label, pageReference->title),\n  "slug": select(\n    pageReference->_type == "homePage" => "inicio",\n    pageReference->_type == "contactPage" => "contato",\n    pageReference->slug.current\n  ),\n  "isDropdown": count(submenu) > 0,\n  "submenu": coalesce(submenu[] {\n    _key,\n    "label": coalesce(label, pageReference->title),\n    "slug": select(\n  pageReference->_type == "homePage" => "inicio",\n  pageReference->_type == "contactPage" => "contato",\n  pageReference->slug.current\n),\n  }, []),\n }, [])\n    },\n    "socialLinks": coalesce(socialLinks[] {\n      platform,\n      url,\n      label,\n      "image": image {\n        \n  _key,\n  hotspot,\n  crop,\n  alt,\n  caption,\n  asset-> {\n    _id,\n    url,\n    altText,\n    metadata {\n      lqip,\n      dimensions {\n        width,\n        height,\n        aspectRatio\n      }\n    }\n  }\n\n      }\n    }, []),\n    "contactInfo": coalesce(contactInfo {\n      address,\n      phones,\n      emails\n    }, { "address": null, "phones": [], "emails": [] })\n  }\n': SITE_SETTINGS_QUERY_RESULT;
    '\n  *[_type == "page" && slug.current == "transparencia"][0] {\n    \n  \n  _id,\n  title,\n  excerpt,\n  \n  "slug": slug.current\n\n,\n  \n  hero {\n    heading,\n    tagline,\n  }\n,\n  "featuredImage": featuredImage { \n  _key,\n  hotspot,\n  crop,\n  alt,\n  caption,\n  asset-> {\n    _id,\n    url,\n    altText,\n    metadata {\n      lqip,\n      dimensions {\n        width,\n        height,\n        aspectRatio\n      }\n    }\n  }\n, showInPage },\n  "pageBuilder": pageBuilder[] { \n  _key,\n  _type,\n  _type == "ctaSection" => {\n    ...,\n    "primaryCta": primaryCta { \n  label,\n  linkType,\n  "slug": select(\n  pageReference->_type == "homePage" => "inicio",\n  pageReference->_type == "contactPage" => "contato",\n  pageReference->slug.current\n),\n  externalUrl,\n  openInNewTab\n },\n    "secondaryCta": secondaryCta { \n  label,\n  linkType,\n  "slug": select(\n  pageReference->_type == "homePage" => "inicio",\n  pageReference->_type == "contactPage" => "contato",\n  pageReference->slug.current\n),\n  externalUrl,\n  openInNewTab\n }\n  },\n  _type == "textWithIllustration" => {\n    ...,\n    "image": image { \n  _key,\n  hotspot,\n  crop,\n  alt,\n  caption,\n  asset-> {\n    _id,\n    url,\n    altText,\n    metadata {\n      lqip,\n      dimensions {\n        width,\n        height,\n        aspectRatio\n      }\n    }\n  }\n },\n    "cta": cta { \n  label,\n  linkType,\n  "slug": select(\n  pageReference->_type == "homePage" => "inicio",\n  pageReference->_type == "contactPage" => "contato",\n  pageReference->slug.current\n),\n  externalUrl,\n  openInNewTab\n }\n  },\n  _type == "gallery" => {\n    ...,\n    "images": images[] { \n  _key,\n  hotspot,\n  crop,\n  alt,\n  caption,\n  asset-> {\n    _id,\n    url,\n    altText,\n    metadata {\n      lqip,\n      dimensions {\n        width,\n        height,\n        aspectRatio\n      }\n    }\n  }\n },\n    autoplay,\n    "autoplayInterval": coalesce(autoplayInterval, 4)\n  },\n  _type == "videoFile" => { \n  ...,\n  "file": {\n    "url": file.asset->url,\n    "mimeType": file.asset->mimeType\n  },\n  "posterImage": posterImage { \n  _key,\n  hotspot,\n  crop,\n  alt,\n  caption,\n  asset-> {\n    _id,\n    url,\n    altText,\n    metadata {\n      lqip,\n      dimensions {\n        width,\n        height,\n        aspectRatio\n      }\n    }\n  }\n }\n },\n  _type == "youtubeEmbed" => { \n  ...,\n  url,\n  caption\n },\n  _type == "horizontalRule" => {\n    ...\n  },\n  _type == "richText" => {\n    ...,\n    content[] { \n  ...,\n  _type == "image" => { \n  _key,\n  hotspot,\n  crop,\n  alt,\n  caption,\n  asset-> {\n    _id,\n    url,\n    altText,\n    metadata {\n      lqip,\n      dimensions {\n        width,\n        height,\n        aspectRatio\n      }\n    }\n  }\n },\n  _type == "videoFile" => { \n  ...,\n  "file": {\n    "url": file.asset->url,\n    "mimeType": file.asset->mimeType\n  },\n  "posterImage": posterImage { \n  _key,\n  hotspot,\n  crop,\n  alt,\n  caption,\n  asset-> {\n    _id,\n    url,\n    altText,\n    metadata {\n      lqip,\n      dimensions {\n        width,\n        height,\n        aspectRatio\n      }\n    }\n  }\n }\n },\n  _type == "youtubeEmbed" => { \n  ...,\n  url,\n  caption\n },\n  _type == "downloadableFile" => { \n  ...,\n  "url": asset->url\n },\n  _type == "projectShowcaseSection" => { \n  ...,\n  items[] {\n    _key,\n    name,\n    description,\n    "cta": cta { \n  label,\n  linkType,\n  "slug": select(\n  pageReference->_type == "homePage" => "inicio",\n  pageReference->_type == "contactPage" => "contato",\n  pageReference->slug.current\n),\n  externalUrl,\n  openInNewTab\n }\n  }\n },\n },\n    background,\n  },\n }\n\n  }\n': TRANSPARENCY_INDEX_PAGE_QUERY_RESULT;
    '\n  *[_type == "transparencySection"] | order(order asc) {\n    \n  _id,\n  title,\n  \n  "slug": slug.current\n,\n  \n  hero {\n    heading,\n    tagline,\n  }\n,\n  partner,\n  excerpt,\n  description[] { \n  ...,\n  _type == "image" => { \n  _key,\n  hotspot,\n  crop,\n  alt,\n  caption,\n  asset-> {\n    _id,\n    url,\n    altText,\n    metadata {\n      lqip,\n      dimensions {\n        width,\n        height,\n        aspectRatio\n      }\n    }\n  }\n },\n  _type == "videoFile" => { \n  ...,\n  "file": {\n    "url": file.asset->url,\n    "mimeType": file.asset->mimeType\n  },\n  "posterImage": posterImage { \n  _key,\n  hotspot,\n  crop,\n  alt,\n  caption,\n  asset-> {\n    _id,\n    url,\n    altText,\n    metadata {\n      lqip,\n      dimensions {\n        width,\n        height,\n        aspectRatio\n      }\n    }\n  }\n }\n },\n  _type == "youtubeEmbed" => { \n  ...,\n  url,\n  caption\n },\n  _type == "downloadableFile" => { \n  ...,\n  "url": asset->url\n },\n  _type == "projectShowcaseSection" => { \n  ...,\n  items[] {\n    _key,\n    name,\n    description,\n    "cta": cta { \n  label,\n  linkType,\n  "slug": select(\n  pageReference->_type == "homePage" => "inicio",\n  pageReference->_type == "contactPage" => "contato",\n  pageReference->slug.current\n),\n  externalUrl,\n  openInNewTab\n }\n  }\n },\n },\n  projects[] { \n  _key,\n  title,\n  origin,\n  value,\n  status,\n  documents[] { \n  _key,\n  title,\n  label,\n  "fileUrl": asset->url,\n  "fileName": asset->originalFilename,\n },\n },\n  documents[] { \n  _key,\n  title,\n  label,\n  "fileUrl": asset->url,\n  "fileName": asset->originalFilename,\n },\n\n  }\n': ALL_TRANSPARENCY_SECTIONS_QUERY_RESULT;
    '\n  *[_type == "transparencySection" && slug.current == $slug][0] {\n    \n  _id,\n  title,\n  \n  "slug": slug.current\n,\n  \n  hero {\n    heading,\n    tagline,\n  }\n,\n  partner,\n  excerpt,\n  description[] { \n  ...,\n  _type == "image" => { \n  _key,\n  hotspot,\n  crop,\n  alt,\n  caption,\n  asset-> {\n    _id,\n    url,\n    altText,\n    metadata {\n      lqip,\n      dimensions {\n        width,\n        height,\n        aspectRatio\n      }\n    }\n  }\n },\n  _type == "videoFile" => { \n  ...,\n  "file": {\n    "url": file.asset->url,\n    "mimeType": file.asset->mimeType\n  },\n  "posterImage": posterImage { \n  _key,\n  hotspot,\n  crop,\n  alt,\n  caption,\n  asset-> {\n    _id,\n    url,\n    altText,\n    metadata {\n      lqip,\n      dimensions {\n        width,\n        height,\n        aspectRatio\n      }\n    }\n  }\n }\n },\n  _type == "youtubeEmbed" => { \n  ...,\n  url,\n  caption\n },\n  _type == "downloadableFile" => { \n  ...,\n  "url": asset->url\n },\n  _type == "projectShowcaseSection" => { \n  ...,\n  items[] {\n    _key,\n    name,\n    description,\n    "cta": cta { \n  label,\n  linkType,\n  "slug": select(\n  pageReference->_type == "homePage" => "inicio",\n  pageReference->_type == "contactPage" => "contato",\n  pageReference->slug.current\n),\n  externalUrl,\n  openInNewTab\n }\n  }\n },\n },\n  projects[] { \n  _key,\n  title,\n  origin,\n  value,\n  status,\n  documents[] { \n  _key,\n  title,\n  label,\n  "fileUrl": asset->url,\n  "fileName": asset->originalFilename,\n },\n },\n  documents[] { \n  _key,\n  title,\n  label,\n  "fileUrl": asset->url,\n  "fileName": asset->originalFilename,\n },\n\n  }\n': TRANSPARENCY_SECTION_BY_SLUG_QUERY_RESULT;
  }
}
